...
/**
 Nil until successful MAF logon registration.
 */
@property (readonly) id<ManagerConfiguratorProtocol> logonConfigurator;
 ...
 */
@property (readonly) MAFApplicationSettings* applicationSettings;
...
/**
 Delete an object from SecureStore for the given key. Synchronous call.
 @param aKey Key for the object
 @param error If the store fails specifies the reason of the error
 @return operation success or not
 */
-(BOOL) deleteObjectFromSecureStoreForKey:(NSString*)aKey error:(NSError**)error;
...
/**
 ...
 Note: deprecated, use setUrl: sourceApplication: method
 */
...
- (MAFLogonCoreState*) logonCoreState;
...
/**
 Call this method if the client identity handled by CertificateProvider is not valid.
 It will call the deleteCertificate method of CertificateProvider and the getCertificate method, to get a new, valid certificate
 When finished, the refreshCertificateFinishedWithError delegate method will be called.
**/
