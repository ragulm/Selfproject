...
/**
 ...
 Example of schedule a media request
 This example shows how to schedule a media download request asynchronously. Implement SODataDownloadMediaDelegate for this scenario.
 
 // media link
 NSURL* mediaLink = [(id<SODataEntity>)entity mediaLink];
 [self.store scheduleMediaDownload:mediaLink delegate:self];
 ...
 //media stream received successfully
 //create image for example
 ...
 Example how to add custom headers to the request
 
 SODataRequestParamSingleDefault *requestParam = [[SODataRequestParamSingleDefault alloc] initWithMode:SODataRequestModeRead resourcePath:@"collectionPath"];
 //add custom header to requestParam
 [[requestParam customHeaders] setValue:@"CustomHeaderValue" forKey:@"CustomHeaderKey"];
 
 [self.onlineStore scheduleRequest:requestParam completion:^(id<SODataRequestExecution> requestExecution, NSError *error) {
    //process response
 }
 ...
 *  Inspect the error once the completion block is executed; nil means no issues
 *  Note that the completion block is redirected to the main UI thread
 *
