...
extern NSString* const kCacheDbResetNotification;
extern NSString* const kNilOrEmptyEncryptionKey;
 ...
 * </pre>
 */
+(BOOL) setEncryptionKey:(NSString *) key withError:(NSError **)error;
...
/**
 * @brief A static method to clear/nullify the key along with the database i.e. by calling this method the DB that exists on the mobile client will be removed. Application/MAF will have to call getEncryptionKey again to generate a new key
 *
 * @return BOOL Indication if the reset was successful
 * @param _error NSError* object to capture any error if occured during the execution of the method. 
 * <pre>
 * Usage
 * <code>
 * NSError* error = nil;
 * BOOL isCleared = [EncryptionKeyManager clearEncryptionKeyWithError:&error];
 * if (!isCleared) NSLog(@"%@", error);     // Printing the error to con
 * </code>
 * </pre>
 */
